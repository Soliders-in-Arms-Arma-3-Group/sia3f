...
private _actionConfirm = [QGVAR(safeStart_missionStartConfirm), "Confirm", "", { remoteExecCall [QEFUNC(core,startMission)]; }, { !(missionNamespace getVariable [QEGVAR(core,missionStarted), false]) }] call ace_interact_menu_fnc_createAction;
...
private _actionEndLose = [QGVAR(safeStart_missionEndLose), "Lose", "", {
	EGVAR(core,missionLost) = true;
