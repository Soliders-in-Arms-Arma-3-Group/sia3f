 ...
 /*
  Author: R3vo, Edited by McKendrick
...
switch (_mode) do
{
  case "createGUI":
  {
    ...
    {
      ["previewPosition"] spawn FUNC(teleport);
    }];
  ...
  case "addActions":
  {
    ...
    _parameters apply
    {
      if ("@ace" call FUNC(checkModPresence)) then { 
        private _action = ["TPD", "Open Teleport Menu", "\a3\modules_f_curator\data\portraitobjectivemove_ca.paa", { findDisplay 46 createDisplay "TPD" }, { true }] call ace_interact_menu_fnc_createAction;
