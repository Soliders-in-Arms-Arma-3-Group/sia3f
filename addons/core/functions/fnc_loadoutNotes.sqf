...
f3_fnc_nameShort = {
	...
	[_name, _type, _prefix, true] call f3_fnc_name
};
...
f3_fnc_loadoutDataToText = {
	...
	if (count _weps > 0) then {
		_textLong = _textLong + "<font size='18'>WEAPONS [#MAGAZINES]:</font>";
		{
			...
			_textLong = _textLong + "<br/>" + ([_weapon] call f3_fnc_name);
			_textShrt = _textShrt + ([_weapon] call f3_fnc_name);

			// Weapon magazines
			_textLong = _textLong + ([_magArr] call f3_fnc_magArrText);
			_textShrt = _textShrt + ([_magArr] call f3_fnc_magArrText);
			...
			// Underslung grenade launcher
			{
				...
				_textLong = _textLong + "<br/>" + _arrow + _picture + "UGL" + ([_x] call f3_fnc_magArrText);
				_textShrt = _textShrt + " + " +  _picture + ([_x] call f3_fnc_magArrText);
			} forEach _ugls;

			// Weapon attachments
			{
				_textLong = _textLong + "<br/>" + ([_x, "CfgWeapons", _arrow] call f3_fnc_name);
				_textShrt = _textShrt + ([_x, "CfgWeapons", " + "] call f3_fnc_nameShort);
	...
	if (_bp != "") then {
		...
		private _bpText = ([_bp, "CfgVehicles"] call f3_fnc_name);
	...
	if (count _mags > 0 || count _items > 0 || count _items_assigned > 0) then {
...

		{
			...
			_textLong = _textLong + "<br/>" + ([_className, "CfgMagazines"] call f3_fnc_name) + format [" [%1]", _num];
			_textShrt = _textShrt + ([_className, "CfgMagazines"] call f3_fnc_nameShort) + format [" [%1]", _num];
		} forEach _mags;

		{
			...
			_textLong = _textLong + "<br/>" + ([_className, "CfgWeapons"] call f3_fnc_name) + format[" [%1]", _num];
			_textShrt = _textShrt + ([_className, "CfgWeapons"] call f3_fnc_nameShort) + format[" [%1]", _num];
		} forEach _items;
		...
		{
			_textLong = _textLong + "<br/>" + ([_x, "CfgWeapons"] call f3_fnc_name);
...
f3_fnc_getLoadout = {
	...
	_weps = _weps apply {
		// Magazines
		private _magArr = [_x, _mags] call f3_fnc_wepMags;
		...
		_ugls = _ugls apply { ["UGL_F", _mags] call f3_fnc_wepMags };
...
private _text = _text_notice + ([player] call f3_fnc_getLoadout);
// ====================================================================================
...
private _text_group = "" + _text_notice;
{
	...
	private _text_loadout = [_x, false] call f3_fnc_getLoadout;
